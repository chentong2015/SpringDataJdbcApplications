...
public interface RoutingRuleRepository extends CrudRepository<RoutingRuleDao, Long> {
    ...
    // select generatedAlias0 from com.example.main.entity.RoutingRuleDao as generatedAlias0
    // where generatedAlias0.ruleElements is empty
    @Query(value = "select * from ddb_routing_rule where not ( exists " +
            "(select * from ddb_routing_rule_element " +
            "where ddb_routing_rule.id=ddb_routing_rule_element.routing_rule_id))", nativeQuery = true)
